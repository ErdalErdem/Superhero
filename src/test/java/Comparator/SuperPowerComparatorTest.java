...
class SuperPowerComparatorTest {
    ...
    void compare() {
        ...
        ArrayList<Superhero> expected = new ArrayList<Superhero>(Arrays.asList(s4, s2, s1, s3)); //Den rigtige rækkefølge
        //Sorterer array list "actual" ifølge vores comparator som så burde give den samme rækkefølge som array list "expected"
        assertEquals(controller.sortSuperPower(actual), expected);
    }
}
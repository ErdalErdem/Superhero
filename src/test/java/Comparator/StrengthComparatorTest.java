...
class StrengthComparatorTest {
    ...
    void compare() {
        ...
        ArrayList<Superhero> expected = new ArrayList<Superhero>(Arrays.asList(s2, s3, s1, s4)); //Den rigtige rækkefølge
        //Sorterer array list "actual" ifølge vores comparator som så burde give den samme rækkefølge som array list "expected"
        assertEquals(controller.sortSuperStrength(actual), expected);
    }
}
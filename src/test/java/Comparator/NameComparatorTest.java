...
class NameComparatorTest {
    ...
    void compare() {
        ...
        Superhero s4 = new Superhero("bjorn", true, "advokat", 1986, 5);
        ArrayList<Superhero> actual = new ArrayList<Superhero>(Arrays.asList(s1, s2, s3, s4));
        ArrayList<Superhero> expected = new ArrayList<Superhero>(Arrays.asList(s2, s4, s1, s3)); //Den rigtige rækkefølge
        //Sorterer array list "actual" ifølge vores comparator som så burde give den samme rækkefølge som array list "expected"
        assertEquals(controller.SortSuperNames(actual), expected);
    }
}